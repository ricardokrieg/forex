...
#property indicator_color2 C'0,90,255'
...
int init()
{
  ...
  SetIndexLabel(2,("T3 Moving Average "+TimeFrameToString(timeFrame)+"("+T3Period+")"));
...
int start()
{
  ...
  for (int x = limit-1; x >= 0; x--)
  {
    ...
    {
      Uptrend[x] = t3Array[x]; 
      if (trend[x+1]<0) Uptrend[x+1]=t3Array[x+1];
    ...
    {
      Dntrend[x] = t3Array[x]; 
      if (trend[x+1]>0) Dntrend[x+1]=t3Array[x+1];
