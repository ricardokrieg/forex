...
//
extern color LabelsColor = Black;
...
extern bool TodayLabelsRight = false;
extern bool SeperateDayLines = false;
...
//
extern bool ShowSupportResistance = false;
extern bool ShowPivotPoint = false;
...
extern color ResistanceColor= LimeGreen; //Exact Dark Red is 139,0,0
extern color PivotColor= Gold;
//
...
int start()
{
  ...
  if (ShowOuterFibs) {
   ...
   // comment= comment + "Pivot Points (C) vS";
      comment= comment + "Range: Y "+DoubleToStr(MathRound(q/Point),0)   +" - T "+DoubleToStr(MathRound(d/Point),0)+" " + "\n";
